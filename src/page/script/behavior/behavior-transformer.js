 ...
 * @returns
 */
function transformBehaviorAst ($ast, fileKey) {
    ...
    $ast
        ...
        .each(item => {
            ...
            if (t.isObjectExpression(arguments)) {
                ...
                global.log(`[ERROR]Behavior异常情况(建议把源代码修改为简单结构，如Behavior({})，再尝试转换)\nfile:${ fileKey }\n`, littleCode)
            }
        })
        .root()
        .replace("Behavior({data:$_$1,$$$})", `Behavior({data() {
            return $_$1;
          },$$$})`)
        .replace('Behavior($_$1)', '($_$1)')  //替换的数据必须加括号，不然分分钟报错。。。。
...
module.exports = { transformBehaviorAst }
