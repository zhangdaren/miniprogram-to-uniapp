	...
	if (isParseError) {
		...
		traverse(javascriptAst, {
			...
			MemberExpression(path) {
				...
				object.node.name = utils.getPropsAlias(object.node.name);
				// console.log("MemberExpression-", object.node.name)
				path.skip();
			},
			ExpressionStatement(path) {
				const expression = path.get("expression");
				expression.node.name = utils.getPropsAlias(expression.node.name);
				// console.log("ExpressionStatement-", expression.node.name)
			},
			BinaryExpression(path) {
				const left = path.get("left");
				left.node.name = utils.getPropsAlias(left.node.name);
