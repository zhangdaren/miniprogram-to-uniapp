...
//
const appVistor = require('./tranfrom/appAstHandle');
const componentVistor = require('./tranfrom/componentAstHandle');
const pageVistor = require('./tranfrom/pageAstHandle');
	...
	ObjectProperty(path) {
		...
		switch (name) {
			...
			case 'properties':
				...
				break;
			case 'lifetimes':
				//组件特有生命周期组lifetimes，不处理，后面自动会遍入lifetimes节点
				break;
			case 'pageLifetimes':
			...
			case 'options':
				//组件的options
				const parent_options = path.parentPath.parent;
				const value_options = parent_options.value;
				if (value_options == dataValue) {
					vistors.data.handle(path.node);
				} else {
					vistors.lifeCycle.handle(path.node);
					path.skip();
...
const componentConverter = function (ast, _miniprogramRoot, _file_js, isVueFile) {
	...
	//判断文件类型
	const astType = babelUtil.getAstType(ast, nodePath.relative(global.targetFolder, file_js));

	//判断是否为app.js
	isAppFile = astType === "App";

	let newAst = ast;
	if (!isVueFile) {
		switch (astType) {
			case "App":
				newAst = traverse(ast, appVistor);
				break;
			case "Page":
				newAst = traverse(ast, pageVistor);
				break;
			case "Component":
			case "VantComponent":
				newAst = traverse(ast, componentVistor);
				break;
			default:
				break;
		}

		// newAst = traverse(ast, componentVistor2);
	}


	return {
		convertedJavascript: newAst,
		vistors: vistors,
